...
// Import telegram notifier (if configured)
let telegram = null;
try {
  telegram = require('../../telegram-notifier.js');
} catch (error) {
  console.log('[Transactional] Telegram notifier not found in parent dir, skipping');
}
...
async function notifyAdmin(message, priority = 'normal') {
  ...
  if (telegram) {
    try {
      await telegram.sendTelegramMessage(message, priority);
      return { success: true, logged: true, telegram: true };
    } catch (error) {
      console.error('[Transactional] Telegram send failed:', error.message);
      // Don't fail if Telegram fails, just log it
    }
  }
  
  return { success: true, logged: true, telegram: false };
}
