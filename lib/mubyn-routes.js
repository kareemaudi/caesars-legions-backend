...
const FALCON_MODEL = process.env.FALCON_MODEL || 'meta-llama/Llama-3.3-70B-Instruct-Turbo';
...
// Falcon chat helper (via Together AI - OpenAI-compatible API)
async function falconChat(systemPrompt, userMessage, maxTokens = 1024) {
  if (!TOGETHER_API_KEY) return null; // fallback to OpenAI if no key
  try {
    ...
    if (!res.ok) { console.error(`Falcon ${res.status}: ${await res.text()}`); return null; }
    const data = await res.json();
    return data.choices[0].message.content;
  } catch (e) { console.error('Falcon error:', e.message); return null; }
}

// Gemini Imagen image generation helper
async function geminiGenerateImage(prompt, aspectRatio = '1:1') {
  if (!GEMINI_API_KEY) return null;
  try {
    // Using Gemini's Imagen 3 model for image generation
    const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/imagen-3.0-generate-001:predict?key=${GEMINI_API_KEY}`, {
      method: 'POST',
  ...
  } catch (e) { console.error('Gemini image error:', e.message); return null; }
}
...
// Smart AI router: Falcon for Arabic, GPT-4o for English
async function smartChat(systemPrompt, userMessage, maxTokens = 1024) {
  const useArabic = isArabic(userMessage);
  let modelUsed = 'gpt-4o';
  
  if (useArabic && TOGETHER_API_KEY) {
    const falconReply = await falconChat(systemPrompt, userMessage, maxTokens);
    if (falconReply) {
      modelUsed = 'falcon-3';
      return { reply: falconReply, model: modelUsed };
    }
  }
  
  // Fallback to GPT-4o
  const reply = await openaiChat(systemPrompt, userMessage, maxTokens);
  return { reply, model: modelUsed };
}

// OpenAI chat with full messages array (for conversation history)
async function openaiChatMessages(messages, maxTokens = 1024) {
  if (!OPENAI_API_KEY) throw new Error('OPENAI_API_KEY not set');
  const res = await fetch('https://api.openai.com/v1/chat/completions', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${OPENAI_API_KEY}` },
    body: JSON.stringify({ model: 'gpt-4o', messages, max_tokens: maxTokens, temperature: 0.7 })
  });
  if (!res.ok) { const err = await res.text(); throw new Error(`OpenAI ${res.status}: ${err}`); }
  const data = await res.json();
  return data.choices[0].message.content;
...
// Falcon chat with full messages array
async function falconChatMessages(messages, maxTokens = 1024) {
  if (!TOGETHER_API_KEY) return null;
  try {
    const res = await fetch('https://api.together.xyz/v1/chat/completions', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${TOGETHER_API_KEY}` },
      body: JSON.stringify({ model: FALCON_MODEL, messages, max_tokens: maxTokens, temperature: 0.7 })
    });
    if (!res.ok) { console.error(`Falcon ${res.status}: ${await res.text()}`); return null; }
    const data = await res.json();
    return data.choices[0].message.content;
...
// Smart AI router with full messages array
async function smartChatMessages(messages, userMessage, maxTokens = 1024) {
  const useArabic = isArabic(userMessage);
  let modelUsed = 'gpt-4o';
  if (useArabic && TOGETHER_API_KEY) {
    const falconReply = await falconChatMessages(messages, maxTokens);
    if (falconReply) return { reply: falconReply, model: 'falcon-3' };
  }
  const reply = await openaiChatMessages(messages, maxTokens);
  return { reply, model: modelUsed };
}
...
const USERID_PATH_PATTERNS = [
  ...
  /^\/integrations\/supabase\/([0-9a-f-]{36})/,
];
...
// POST /settings/smtp/test - Verify SMTP credentials
router.post('/settings/smtp/test', async (req, res) => {
  try {
    const userId = req.user?.id || req.body.userId;
    if (!userId) return res.status(400).json({ error: 'userId required' });
    const smtpData = await loadJSON(path.join(DATA_DIR, `smtp-${userId}.json`), null);
    if (!smtpData || !smtpData.email) return res.status(400).json({ error: 'No SMTP configured', connected: false });
    const decipher = require('crypto').createDecipheriv('aes-256-cbc',
      Buffer.from((process.env.ENCRYPTION_KEY || 'mubyn-default-encryption-key-32b').padEnd(32, '0').slice(0, 32)),
      Buffer.alloc(16, 0));
    let password = decipher.update(smtpData.password_encrypted || '', 'hex', 'utf8');
    password += decipher.final('utf8');
    const testTransport = nodemailer.createTransport({
      host: smtpData.host || 'smtp.zoho.com', port: smtpData.port || 587,
      secure: (smtpData.port || 587) === 465, auth: { user: smtpData.email, pass: password },
      connectionTimeout: 8000
    });
    await testTransport.verify();
    res.json({ success: true, connected: true, email: smtpData.email, host: smtpData.host });
  } catch (e) {
...
// ═══════════════════════════════════════════════════════════════════
// CHAT — Caesar Agent with Memory, Tools, and Full Business Context
// Features: conversation history, rich context injection, tool calling,
//           memory extraction (learns over time)
// ═══════════════════════════════════════════════════════════════════

// ── Helper: Build rich business context string from all user data ──
async function buildBusinessContext(userId) {
  const contextParts = [];
  try {
    // Load ALL user data in parallel
    const [userData, leads, content, cfo, csaKb, smtp, shopify, meta, memoryFacts] = await Promise.all([
      ...
      loadJSON(path.join(DATA_DIR, `csa-kb-${userId}.json`), []),
      loadJSON(path.join(DATA_DIR, `smtp-${userId}.json`), null),
      loadJSON(path.join(DATA_DIR, `integrations-shopify-${userId}.json`), null),
      loadJSON(path.join(DATA_DIR, `integrations-meta-${userId}.json`), null),
      loadJSON(path.join(DATA_DIR, `memory-${userId}.json`), []),
    ]);

    // Business info
    if (userData) {
      const info = [];
      if (userData.business_name) info.push(userData.business_name);
      if (userData.industry) info.push(userData.industry);
      if (userData.country) info.push(`in ${userData.country}`);
      if (userData.website) info.push(`website: ${userData.website}`);
      if (info.length) contextParts.push(`BUSINESS: ${info.join(', ')}`);
    }

    // Leads pipeline
    if (leads.length > 0) {
      const recentNames = leads.slice(0, 5).map(l => l.businessName || l.contactName).filter(Boolean).join(', ');
      const statusCounts = {};
      ...
      contextParts.push(`LEADS: ${leads.length} total (${statusStr}). Recent: ${recentNames}`);
    } else {
      contextParts.push('LEADS: None yet — suggest generating leads');
    }

    // CFO / Financial
    if (cfo) {
      const rev = cfo.totalRevenue || cfo.monthlyRevenue || 0;
      const exp = cfo.totalExpenses || cfo.monthlyExpenses || 0;
      const profit = cfo.netProfit || 0;
      contextParts.push(`FINANCIALS: Revenue $${rev.toLocaleString()}, Expenses $${Math.abs(exp).toLocaleString()}, Net Profit $${profit.toLocaleString()} (${cfo.profitMargin || 'N/A'} margin)`);
      if (cfo.categoryBreakdown) {
        const cats = Object.entries(cfo.categoryBreakdown)
          .map(([cat, data]) => `${cat}: $${(data.income||0).toLocaleString()} in / $${Math.abs(data.expenses||0).toLocaleString()} out`)
          .join('; ');
        contextParts.push(`FINANCIAL BREAKDOWN: ${cats}`);
      }
    } else {
      contextParts.push('FINANCIALS: Not set up yet');
    }
    ...
    if (content.length > 0) {
      ...
      contextParts.push(`CONTENT: ${content.length} posts (${drafts} drafts, ${published} published)`);
    } else {
      contextParts.push('CONTENT: No posts yet');
    }

    // CS Knowledge Base
    if (csaKb.length > 0) {
      contextParts.push(`CUSTOMER SUPPORT: ${csaKb.length} KB articles`);
    } else {
      contextParts.push('CUSTOMER SUPPORT: No KB articles yet');
    }

    // Email / SMTP
    contextParts.push(`EMAIL: ${smtp && smtp.email ? `Connected (${smtp.email})` : 'Not connected'}`);

    // Integrations
    contextParts.push(`SHOPIFY: ${shopify && shopify.connected ? 'Connected' : 'Not connected'}`);
    contextParts.push(`META ADS: ${meta && meta.connected ? 'Connected' : 'Not connected'}`);

    // Memory facts (persistent learned knowledge)
    if (memoryFacts.length > 0) {
      contextParts.push(`MEMORY (facts I've learned about this client):\n${memoryFacts.map(f => `- ${f}`).join('\n')}`);
    }
  } catch (err) {
...
// ── Helper: Load conversation history as OpenAI messages ──
async function loadConversationHistory(userId, limit = 20) {
  try {
    const convFile = path.join(DATA_DIR, `conversations-${userId}.json`);
    const conversations = await loadJSON(convFile, []);
...
// ── Helper: Parse and execute action tags from AI response ──
async function executeActions(responseText, userId) {
  const actionRegex = /\[ACTION:([A-Z_]+)(?::([^\]]*))?\]/g;
  const actions = [];
  let match;
  while ((match = actionRegex.exec(responseText)) !== null) {
    actions.push({ tag: match[0], name: match[1], params: match[2] || '' });
  }
  if (actions.length === 0) return { cleanResponse: responseText, toolResults: [] };
    ...
    try {
      switch (action.name) {
        case 'GENERATE_LEADS': {
          ...
          const location = parts[1] || 'Dubai';
          const count = parseInt(parts[2]) || 5;
          // Call lead generation logic directly
          const numLeads = Math.min(Math.max(count, 1), 20);
          const sysPrompt = `You are a business research assistant. Find REAL, currently operating businesses. Return ONLY a valid JSON array. Each object: { "businessName": "str", "contactName": "str", "contactTitle": "str", "phone": "str", "website": "str", "address": "str", "googleRating": number, "industry": "str", "description": "str" }`;
          const userMsg = `Find ${numLeads} real ${industry} businesses in ${location}. Mix of well-known and smaller local businesses.`;
          const rawResponse = await openaiChat(sysPrompt, userMsg, numLeads * 250);
          const businesses = parseAIJson(rawResponse);
          if (Array.isArray(businesses) && businesses.length > 0) {
            const leads = businesses.map(b => ({
              id: crypto.randomUUID(),
              businessName: b.businessName || b.business_name || 'Unknown',
              contactName: b.contactName || b.contact_name || 'N/A',
              contactTitle: b.contactTitle || b.contact_title || 'Manager',
              email: 'N/A',
              phone: b.phone || 'N/A',
              website: b.website || 'N/A',
              address: b.address || location,
              googleRating: parseFloat(b.googleRating || 4.0),
            ...
            for (const lead of leads) {
              const website = (lead.website || '').replace(/^https?:\/\//, '').replace(/^www\./, '').split('/')[0];
              if (lead.contactName !== 'N/A' && website && website.includes('.')) {
                const nameParts = lead.contactName.trim().split(/\s+/);
                if (nameParts.length >= 2) {
                  const patterns = generatePatterns(nameParts[0], nameParts.slice(1).join(' '), website, { companySize: 'smb' });
                  lead.email = patterns[0] || `info@${website}`;
                  lead.emailPatterns = patterns;
                } else {
                  lead.email = `${nameParts[0].toLowerCase()}@${website}`;
                }
              } else if (website && website.includes('.')) {
                lead.email = `info@${website}`;
              }
            }
            // Save
            const leadsFile = path.join(DATA_DIR, `leads-${userId}.json`);
            const existing = await loadJSON(leadsFile, []);
            await saveJSON(leadsFile, [...leads, ...existing]);
            toolResults.push({ action: 'GENERATE_LEADS', success: true, count: leads.length, summary: `Generated ${leads.length} ${industry} leads in ${location}: ${leads.slice(0, 3).map(l => l.businessName).join(', ')}${leads.length > 3 ? '...' : ''}` });
          } else {
            toolResults.push({ action: 'GENERATE_LEADS', success: false, error: 'Could not parse lead results' });
          }
          break;
        }

        case 'GET_CFO': {
          const cfo = await loadJSON(path.join(DATA_DIR, `cfo-${userId}.json`), null);
          if (cfo) {
            toolResults.push({
              action: 'GET_CFO', success: true,
              summary: `Revenue: $${(cfo.totalRevenue||cfo.monthlyRevenue||0).toLocaleString()}, Expenses: $${(cfo.totalExpenses||cfo.monthlyExpenses||0).toLocaleString()}, Net Profit: $${(cfo.netProfit||0).toLocaleString()} (${cfo.profitMargin || 'N/A'} margin)`,
              data: { revenue: cfo.totalRevenue||cfo.monthlyRevenue||0, expenses: cfo.totalExpenses||cfo.monthlyExpenses||0, netProfit: cfo.netProfit||0, margin: cfo.profitMargin }
            });
          } else {
            toolResults.push({ action: 'GET_CFO', success: false, error: 'No financial data found. Generate projections in the CFO tab first.' });
          }
          break;
        }
        ...
        case 'GET_LEADS': {
          ...
          if (leads.length > 0) {
            ...
            leads.forEach(l => { statusCounts[l.status || 'new'] = (statusCounts[l.status || 'new'] || 0) + 1; });
            const statusStr = Object.entries(statusCounts).map(([s, c]) => `${c} ${s}`).join(', ');
            const recentNames = leads.slice(0, 5).map(l => l.businessName).join(', ');
            toolResults.push({
              action: 'GET_LEADS', success: true,
              summary: `${leads.length} leads total (${statusStr}). Recent: ${recentNames}`,
              data: { total: leads.length, statuses: statusCounts }
            });
          } else {
            toolResults.push({ action: 'GET_LEADS', success: false, error: 'No leads found. Ask me to generate some!' });
          }
          break;
        }
        ...
        case 'GENERATE_CONTENT': {
          const parts = action.params.split(':');
          const platform = parts[0] || 'linkedin';
          const topic = parts.slice(1).join(':') || 'business growth';
          const contentText = await openaiChat(
            'You are a social media content expert. Generate engaging platform-appropriate posts.',
            `Create a ${platform} post about: ${topic}. Make it engaging and professional.`, 512
          );
          const item = { id: crypto.randomUUID(), content: contentText, topic, platform, language: 'en', status: 'draft', created_at: new Date().toISOString() };
          const contentFile = path.join(DATA_DIR, `content-${userId}.json`);
          const existing = await loadJSON(contentFile, []);
          existing.push(item);
          await saveJSON(contentFile, existing);
          toolResults.push({
            action: 'GENERATE_CONTENT', success: true,
            summary: `Created ${platform} post about "${topic}": "${contentText.substring(0, 100)}..."`,
            data: { platform, topic, contentPreview: contentText.substring(0, 200) }
          });
          break;
        }

        default:
          toolResults.push({ action: action.name, success: false, error: `Unknown action: ${action.name}` });
      }
    } catch (err) {
      console.error(`Action ${action.name} error:`, err.message);
      toolResults.push({ action: action.name, success: false, error: err.message });
    }
  }

  // Strip action tags from the visible response
  let cleanResponse = responseText;
  for (const action of actions) {
    cleanResponse = cleanResponse.replace(action.tag, '').trim();
  }

  // Append tool result summaries
  if (toolResults.length > 0) {
    const resultSummaries = toolResults
      .filter(r => r.success)
      .map(r => `✅ **${r.action}**: ${r.summary}`)
      .join('\n');
    const errorSummaries = toolResults
      .filter(r => !r.success)
      .map(r => `⚠️ **${r.action}**: ${r.error}`)
      .join('\n');
    if (resultSummaries) cleanResponse += '\n\n---\n' + resultSummaries;
    if (errorSummaries) cleanResponse += '\n' + errorSummaries;
  }

  return { cleanResponse, toolResults };
}

// ── Helper: Extract memory facts from conversation ──
async function extractMemoryFacts(userId, userMessage, aiResponse) {
  try {
    const memoryFile = path.join(DATA_DIR, `memory-${userId}.json`);
    const existingFacts = await loadJSON(memoryFile, []);

    const extractPrompt = `Extract any key business facts, preferences, decisions, or personal details from this conversation that would be useful to remember for future interactions. Return as a JSON array of strings. Only extract genuinely NEW information — not generic chat. If nothing noteworthy, return an empty array [].
...
Assistant: ${aiResponse}`;
    ...
    const rawFacts = await openaiChat(
      'You extract key business facts from conversations. Return ONLY a valid JSON array of strings. Be selective — only genuinely new, specific information.',
      extractPrompt, 300
    );

    const newFacts = parseAIJson(rawFacts);
    if (Array.isArray(newFacts) && newFacts.length > 0) {
      const updatedFacts = [...existingFacts, ...newFacts].slice(-50); // Keep last 50
      await saveJSON(memoryFile, updatedFacts);
      console.log(`[Memory] Extracted ${newFacts.length} facts for user ${userId}: ${newFacts.join('; ')}`);
    }
  } catch (err) {
...
router.post('/chat', async (req, res) => {
  try {
    ...
    let businessContext = '';
    if (uid) {
      businessContext = await buildBusinessContext(uid);
    }

    // ── 2. Load conversation history (last 20 messages) ──
    let historyMessages = [];
    if (uid) {
      historyMessages = await loadConversationHistory(uid, 20);
    }

    // ── 3. Build system prompt with context + tool instructions ──
...
You are an AUTONOMOUS AGENT with memory and tools. You remember past conversations and can take actions on behalf of the user.

You manage 7 departments for each client:
1. **Lead Generation** — AI discovers real businesses, scores leads, drafts personalized emails, sends via SMTP
2. **CMO / Marketing** — Analytics dashboard, content calendar, campaign tracking. Shopify + Meta Ads + Google Ads data.
3. **Financial Intelligence** — Revenue tracking, expense management, projections, AI insights
4. **Customer Support** — AI agent with knowledge base, 5 channels: widget, email, Telegram, WhatsApp, live chat
5. **Website Builder** — AI builds complete websites in 60 seconds
6. **Integrations** — Shopify, Meta Ads, Google Ads connected data
7. **Settings** — Business info, SMTP email, integrations, billing

${businessContext ? `\n═══ CURRENT BUSINESS STATE ═══\n${businessContext}\n═══════════════════════════` : ''}

YOUR BEHAVIOR:
- You have FULL MEMORY of past conversations. Reference previous discussions naturally.
- If user just signed up (no leads, no content): Welcome warmly, offer to generate leads immediately.
- If modules not set up: Proactively suggest next steps.
- Be concise (2-4 paragraphs max), actionable, and warm.
- MATCH THE USER'S LANGUAGE — Arabic → Arabic, English → English, Mixed → bilingual.
- Never say "I'm just an AI" — you ARE their COO running their business 24/7.
- Use markdown formatting. Reference specific data when relevant.

═══ TOOL SYSTEM ═══
You can TAKE ACTIONS for the user. When the user asks you to do something, DO IT by including action tags at the END of your response (after your message). The system will execute them automatically.

Available actions:
- [ACTION:GENERATE_LEADS:industry:location:count] — Generate real business leads. Example: [ACTION:GENERATE_LEADS:restaurants:Dubai:10]
- [ACTION:GET_CFO] — Fetch current financial data
- [ACTION:GET_LEADS] — Get current leads summary
- [ACTION:GENERATE_CONTENT:platform:topic] — Create a social media post. Example: [ACTION:GENERATE_CONTENT:linkedin:AI in business]

RULES for actions:
- Include action tags ONLY when the user explicitly asks you to do something (generate leads, check finances, create content, etc.)
- Place ALL action tags at the very end of your response, each on its own line
- You can include MULTIPLE actions in one response
- Write your conversational response FIRST, then the action tags
- Do NOT mention the action tags in your visible message — the system handles them
═══════════════`;

    // ── 4. Build messages array: system + history + current message ──
    const messages = [{ role: 'system', content: systemPrompt }];
    // Add conversation history (but cap total context to avoid token overflow)
    // Each history message is ~100 tokens avg, 40 messages (20 pairs) = ~4000 tokens
    messages.push(...historyMessages);
    messages.push({ role: 'user', content: message });

    // ── 5. Call AI with full conversation context ──
    const { reply: rawReply, model: aiModel } = await smartChatMessages(messages, message, 1500);

    // ── 6. Parse and execute any action tags ──
    let finalResponse = rawReply;
    let toolResults = [];
    if (uid) {
      const actionResult = await executeActions(rawReply, uid);
      finalResponse = actionResult.cleanResponse;
      toolResults = actionResult.toolResults;
    }

    // ── 7. Save conversation ──
    if (uid) {
      const f = path.join(DATA_DIR, `conversations-${uid}.json`);
      const c = await loadJSON(f, []);
      c.push({
        id: crypto.randomUUID(), userId: uid, message, response: finalResponse,
        model: aiModel, toolResults: toolResults.length > 0 ? toolResults : undefined,
        timestamp: new Date().toISOString()
      });
      await saveJSON(f, c);
    }

    // ── 8. Extract memory facts (async, non-blocking) ──
    if (uid) {
      extractMemoryFacts(uid, message, finalResponse).catch(err => {
        console.error('Background memory extraction failed:', err.message);
      });
    }

    // ── 9. Return response ──
    const responsePayload = { response: finalResponse, model: aiModel };
    if (toolResults.length > 0) responsePayload.toolResults = toolResults;
...
router.post('/chat/stream', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are Caesar (قيصر), the AI COO powering Mubyn OS — an AI business operating system for SMEs in MENA.
${businessContext ? `\n═══ CURRENT BUSINESS STATE ═══\n${businessContext}\n═══════════════════════════` : ''}

BEHAVIOR: Be concise (2-4 paragraphs), actionable, warm. Match user's language. Never say "I'm just an AI" — you ARE their COO.`;
    ...
    const apiKey = useArabic && TOGETHER_API_KEY ? TOGETHER_API_KEY : OPENAI_API_KEY;
    const apiUrl = useArabic && TOGETHER_API_KEY 
      ? 'https://api.together.xyz/v1/chat/completions'
      : 'https://api.openai.com/v1/chat/completions';
    const model = useArabic && TOGETHER_API_KEY ? FALCON_MODEL : 'gpt-4o';
    ...
    res.write(`data: ${JSON.stringify({ type: 'model', model: useArabic ? 'falcon' : 'gpt-4o' })}\n\n`);
    ...
    // Save conversation (same as regular chat)
    if (uid && fullResponse) {
      const f = path.join(DATA_DIR, `conversations-${uid}.json`);
      const c = await loadJSON(f, []);
      c.push({
        id: crypto.randomUUID(), userId: uid, message, response: fullResponse,
        model: useArabic ? 'falcon' : 'gpt-4o', streaming: true,
        timestamp: new Date().toISOString()
      });
      await saveJSON(f, c);
    }
...
router.get('/ai/models', (req, res) => {
  res.json({
    models: [
      { id: 'gpt-4o', name: 'GPT-4o', provider: 'OpenAI', language: 'English', active: !!OPENAI_API_KEY },
      { id: 'falcon-3', name: 'Falcon 3', provider: 'TII Abu Dhabi 🇦🇪', language: 'Arabic', active: !!TOGETHER_API_KEY, note: 'UAE-built, Arabic-native' }
    ],
    routing: 'auto-detect',
    description: 'Model-agnostic: Falcon for Arabic, GPT-4o for English. Auto-detects language.'
  });
});
...
CRITICAL RULE: NEVER fabricate claims, awards, case studies, statistics, or social proof that don't exist. Do NOT say "we've helped 200+ companies" or "we were recently featured" or "our clients see 40% growth" unless the sender explicitly provided those facts. If you don't have real proof points, focus on asking smart questions about THEIR business instead.
...
- FAKE social proof or made-up statistics
- Claiming awards, press, or case studies that weren't provided
...
- Ask a smart question about a challenge in their industry
- Offer a specific, actionable insight or observation about their business
...
- Be honest: "I'm reaching out because..." is fine
...
// CONTENT IMAGE GENERATION (Gemini Imagen 3 → DALL-E 3 fallback)
router.post('/content/image', async (req, res) => {
  try {
    const { prompt, size, platform, provider } = req.body;
    if (!prompt) return res.status(400).json({ error: 'Prompt required' });

    // World-class prompt enhancement via GPT-4o
    const enhancedPrompt = await openaiChat(
      `You are an elite creative director and prompt engineer for AI image generation. Your job is to transform basic social media post descriptions into stunning, photorealistic image prompts that look like they were shot by a professional photographer or designed by a top agency.
...
- Keep it to 2-3 sentences max`,
      `Enhance this into a world-class AI image generation prompt: "${prompt}"`,
      300
    );

    // Platform-aware aspect ratio for Gemini, size for DALL-E
    const aspectRatio = platform === 'instagram' ? '1:1' : '16:9';
    const imageSize = size || (platform === 'instagram' ? '1024x1024' : '1792x1024');

    // Try Gemini Imagen 3 first (better quality, native text rendering)
    if (GEMINI_API_KEY && provider !== 'dalle') {
      const geminiResult = await geminiGenerateImage(enhancedPrompt, aspectRatio);
      if (geminiResult) {
        return res.json({ 
          success: true, 
          image_url: geminiResult, 
          enhanced_prompt: enhancedPrompt,
          provider: 'gemini-imagen-3'
        });
      }
      console.log('Gemini Imagen failed, falling back to DALL-E');
    }

    // Fallback to DALL-E 3
    if (!OPENAI_API_KEY) return res.status(500).json({ error: 'No image generation API configured' });
    ...
    res.json({ success: true, image_url: imageUrl, revised_prompt: revisedPrompt, enhanced_prompt: enhancedPrompt, provider: 'dalle-3' });
...
// ═══════════════════════════════════════════════════════════════════
// SUPABASE / CUSTOM API INTEGRATION (Enterprise Tier - READ ONLY)
// ═══════════════════════════════════════════════════════════════════

// POST /integrations/supabase/connect — Test & save Supabase connection
router.post('/integrations/supabase/connect', async (req, res) => {
  try {
    const userId = req.body.userId || req.user.id;
    const { supabaseUrl, supabaseKey, projectName } = req.body;
    ...
    // Test connection by fetching OpenAPI schema (lists tables)
    let tables = [];
    try {
      ...
      if (!testRes.ok) {
        ...
        return res.status(400).json({ error: `Supabase connection failed (${testRes.status}): ${errText.slice(0, 200)}` });
      }
      const schema = await testRes.json();
    ...
    const filePath = path.join(DATA_DIR, `integration-supabase-${userId}.json`);
    await saveJSON(filePath, connectionData);

    res.json({ success: true, projectName: connectionData.projectName, tables, connectedAt: connectionData.connectedAt });
  } catch (e) {
...
// PATCH /integrations/supabase/:userId/key — Update Supabase API key (e.g. switch to service_role)
router.patch('/integrations/supabase/:userId/key', async (req, res) => {
  try {
    const userId = req.params.userId;
    if (req.user && req.user.id !== userId) return res.status(403).json({ error: 'Access denied' });
    ...
    if (!data || data.status !== 'connected') return res.status(400).json({ error: 'No connection found' });
    data.supabaseKey = supabaseKey;
    data.keyUpdatedAt = new Date().toISOString();
    await saveJSON(filePath, data);
    res.json({ success: true, message: 'API key updated' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// GET /integrations/supabase/:userId/status — Connection status
router.get('/integrations/supabase/:userId/status', async (req, res) => {
  try {
    const userId = req.params.userId;
    const filePath = path.join(DATA_DIR, `integration-supabase-${userId}.json`);
    const data = await loadJSON(filePath, null);
...
// GET /integrations/supabase/:userId/data/:table — READ ONLY passthrough to Supabase REST
router.get('/integrations/supabase/:userId/data/:table', async (req, res) => {
  try {
    const userId = req.params.userId;
    ...
    const data = await loadJSON(filePath, null);
    if (!data || data.status !== 'connected') {
      return res.status(400).json({ error: 'No Supabase connection found' });
    }

    // Validate table is in discovered list
    if (data.tables && !data.tables.includes(table)) {
      return res.status(404).json({ error: `Table "${table}" not found in connected Supabase` });
    }

    // Build query params for Supabase REST
    const queryParams = new URLSearchParams();
    if (req.query.select) queryParams.set('select', req.query.select);
    if (req.query.order) queryParams.set('order', req.query.order);
    // Supabase uses Range header for limit/offset
    const limit = parseInt(req.query.limit) || 100;
    const offset = parseInt(req.query.offset) || 0;

    const supaRes = await fetch(`${data.supabaseUrl}/rest/v1/${encodeURIComponent(table)}?${queryParams.toString()}`, {
      method: 'GET',
      headers: {
    ...
    if (!supaRes.ok) {
      ...
      return res.status(supaRes.status).json({ error: `Supabase query failed: ${errText.slice(0, 200)}` });
    }
    ...
    res.json({ table, rows, total, limit, offset });
  } catch (e) {
    console.error('Supabase data read error:', e);
    res.status(500).json({ error: 'Failed to read Supabase data' });
  }
});

// POST /integrations/supabase/:userId/sync — Sync Supabase data into Mubyn modules
router.post('/integrations/supabase/:userId/sync', async (req, res) => {
  try {
    const userId = req.params.userId;
    const filePath = path.join(DATA_DIR, `integration-supabase-${userId}.json`);
    const data = await loadJSON(filePath, null);
    ...
    async function fetchTable(table, limit = 1000) {
      const r = await fetch(`${data.supabaseUrl}/rest/v1/${encodeURIComponent(table)}?limit=${limit}`, {
    ...
    if (tables.includes('clients')) {
      try {
        ...
        for (const c of clients) {
          ...
          if (email && existingEmails.has(email)) continue; // skip dupes
          existingLeads.push({
            id: crypto.randomUUID(),
            businessName: c.name || c.business_name || c.company_name || '',
            email: email,
            contactName: c.contact_name || c.full_name || c.name || '',
        ...
        results.synced.push({ table: 'clients', type: 'leads', count: added });
      } catch (e) {
        results.errors.push({ table: 'clients', error: e.message });
      }
    }

    // 2. invoices → CFO transactions
    if (tables.includes('invoices')) {
      try {
        const invoices = await fetchTable('invoices');
        const txFile = path.join(DATA_DIR, `cfo-transactions-${userId}.json`);
        const existingTx = await loadJSON(txFile, []);
        const existingIds = new Set(existingTx.map(t => t.externalId));
        let added = 0;
        for (const inv of invoices) {
          const extId = `supabase-inv-${inv.id}`;
          if (existingIds.has(extId)) continue;
          existingTx.push({
            id: crypto.randomUUID(),
            externalId: extId,
            amount: parseFloat(inv.amount || inv.total || 0),
            type: inv.status || inv.type || 'invoice',
        ...
        await saveJSON(txFile, existingTx);
        results.synced.push({ table: 'invoices', type: 'cfo-transactions', count: added });
      } catch (e) {
        results.errors.push({ table: 'invoices', error: e.message });
      }
    }

    // 3. projects → stored as integration data
    if (tables.includes('projects')) {
      try {
        const projects = await fetchTable('projects');
        const projFile = path.join(DATA_DIR, `integration-supabase-projects-${userId}.json`);
        await saveJSON(projFile, { projects, syncedAt: new Date().toISOString() });
        results.synced.push({ table: 'projects', type: 'projects-data', count: projects.length });
      } catch (e) {
        results.errors.push({ table: 'projects', error: e.message });
      }
    }

    // 4. team_members OR employees OR payroll_eligible_employees → stored for context
    const teamTable = tables.includes('payroll_eligible_employees') ? 'payroll_eligible_employees'
      : tables.includes('employees') ? 'employees'
      : tables.includes('team_members') ? 'team_members' : null;
    if (teamTable) {
      try {
        const members = await fetchTable(teamTable);
        const teamFile = path.join(DATA_DIR, `integration-supabase-team-${userId}.json`);
        await saveJSON(teamFile, { team_members: members, source_table: teamTable, syncedAt: new Date().toISOString() });
        results.synced.push({ table: teamTable, type: 'team-context', count: members.length });

        // If payroll data, enrich CFO with real expenses
        if (teamTable === 'payroll_eligible_employees' && members.length > 0) {
          try {
            const totalPayroll = members.reduce((sum, m) => sum + (parseFloat(m.amount) || 0), 0);
            const cfoFile = path.join(DATA_DIR, `cfo-${userId}.json`);
            const cfo = await loadJSON(cfoFile, {});
            cfo.monthlyExpenses = totalPayroll;
            cfo.netProfit = (cfo.monthlyRevenue || 0) - totalPayroll;
            cfo.profitMargin = cfo.monthlyRevenue ? `${Math.round(((cfo.monthlyRevenue - totalPayroll) / cfo.monthlyRevenue) * 100)}%` : '0%';
            cfo.kpis = cfo.kpis || {};
            cfo.kpis.burnRate = totalPayroll;
            cfo.kpis.cashRunway = cfo.monthlyRevenue ? `${Math.round((cfo.monthlyRevenue * 3) / totalPayroll)} months` : 'N/A';
            // Add payroll as transactions
            cfo.transactions = cfo.transactions || [];
            const payrollTxns = members.filter(m => m.amount > 0).map(m => ({
              id: `payroll-${m.id}`, date: new Date().toISOString().slice(0, 10),
              type: 'expense', category: 'Payroll',
              description: `${m.name || 'Employee'} - ${m.position || m.role || 'Team'}`,
              amount: -parseFloat(m.amount)
            }));
            // Replace old payroll txns, keep non-payroll
            cfo.transactions = [...cfo.transactions.filter(t => !t.id?.startsWith('payroll-')), ...payrollTxns];
            cfo.dataSource = 'supabase-sync';
            cfo.lastSyncAt = new Date().toISOString();
            await saveJSON(cfoFile, cfo);
            results.synced.push({ table: 'payroll→CFO', type: 'cfo-expenses', count: members.length, totalPayroll });
          } catch (cfoErr) { results.errors.push({ table: 'payroll→CFO', error: cfoErr.message }); }
        }
      } catch (e) {
        results.errors.push({ table: teamTable, error: e.message });
      }
    }

    // 5. financial_transactions → CFO (REAL credits & debits)
    if (tables.includes('financial_transactions')) {
      try {
        const txns = await fetchTable('financial_transactions');
        const cfoFile = path.join(DATA_DIR, `cfo-${userId}.json`);
        const cfo = await loadJSON(cfoFile, {});
        ...
        const mappedTxns = txns.map(t => ({
          ...
          source: 'supabase-financial'
        }));

        // Calculate real P&L
        const totalIncome = mappedTxns.filter(t => t.amount > 0).reduce((s, t) => s + t.amount, 0);
        const totalExpenses = mappedTxns.filter(t => t.amount < 0).reduce((s, t) => s + t.amount, 0);
        const netProfit = totalIncome + totalExpenses;

        // Monthly breakdown for current year
        const now = new Date();
        const currentYear = now.getFullYear().toString();
        const thisYearTxns = mappedTxns.filter(t => t.date.includes(currentYear));
        const monthCount = Math.max(1, now.getMonth() + 1);
        const avgMonthlyRevenue = Math.round(thisYearTxns.filter(t => t.amount > 0).reduce((s, t) => s + t.amount, 0) / monthCount);
        const avgMonthlyExpenses = Math.round(Math.abs(thisYearTxns.filter(t => t.amount < 0).reduce((s, t) => s + t.amount, 0)) / monthCount);

        // Category breakdown
        const categoryBreakdown = {};
        for (const t of mappedTxns) {
          if (!categoryBreakdown[t.category]) categoryBreakdown[t.category] = { income: 0, expenses: 0, count: 0 };
          if (t.amount >= 0) categoryBreakdown[t.category].income += t.amount;
          else categoryBreakdown[t.category].expenses += t.amount;
          categoryBreakdown[t.category].count++;
        }

        // Update CFO data
        cfo.totalRevenue = totalIncome;
        cfo.totalExpenses = Math.abs(totalExpenses);
        cfo.netProfit = netProfit;
        cfo.profitMargin = totalIncome > 0 ? `${Math.round((netProfit / totalIncome) * 100)}%` : '0%';
        cfo.monthlyRevenue = avgMonthlyRevenue;
        cfo.monthlyExpenses = avgMonthlyExpenses;
        cfo.transactionCount = txns.length;
        cfo.categoryBreakdown = categoryBreakdown;
        cfo.kpis = cfo.kpis || {};
        cfo.kpis.totalRevenue = `$${totalIncome.toLocaleString()}`;
        cfo.kpis.totalExpenses = `$${Math.abs(totalExpenses).toLocaleString()}`;
        cfo.kpis.netProfit = `$${netProfit.toLocaleString()}`;
        cfo.kpis.avgMonthlyRevenue = `$${avgMonthlyRevenue.toLocaleString()}`;
        cfo.kpis.transactionCount = txns.length;

        // Store ALL transactions (replace old financial ones, keep payroll)
        cfo.transactions = [
          ...(cfo.transactions || []).filter(t => !t.id?.startsWith('ft-')),
          ...mappedTxns.slice(0, 200) // Keep latest 200 for display
        ];
        cfo.dataSource = 'supabase-financial-transactions';
        cfo.lastSyncAt = new Date().toISOString();
        await saveJSON(cfoFile, cfo);

        results.synced.push({ table: 'financial_transactions', type: 'cfo-full', count: txns.length, totalIncome, totalExpenses: Math.abs(totalExpenses), netProfit });
      } catch (e) {
        results.errors.push({ table: 'financial_transactions', error: e.message });
      }
    }

    // 6. pending_invoice_jobs → CFO Transactions
    if (tables.includes('pending_invoice_jobs')) {
      try {
        const jobs = await fetchTable('pending_invoice_jobs');
        const txFile = path.join(DATA_DIR, `cfo-transactions-${userId}.json`);
        const existingTx = await loadJSON(txFile, []);
        const existingIds = new Set(existingTx.map(t => t.externalId));
        let added = 0;
        for (const job of jobs) {
          const extId = `supabase-invjob-${job.id}`;
          if (existingIds.has(extId)) continue;
          existingTx.push({
            id: crypto.randomUUID(),
            externalId: extId,
            amount: parseFloat(job.amount || job.total || 0),
            type: 'income',
            category: 'Invoice',
            description: `Invoice Job #${job.id} — ${job.status || 'pending'}`,
            date: job.created_at || job.date || new Date().toISOString(),
            source: 'supabase'
          });
          added++;
        }
        await saveJSON(txFile, existingTx);
        results.synced.push({ table: 'pending_invoice_jobs', type: 'cfo-transactions', count: added });
      } catch (e) {
        results.errors.push({ table: 'pending_invoice_jobs', error: e.message });
      }
    }
    ...
    res.json({ success: true, results, lastSyncAt: data.lastSyncAt });
  } catch (e) {
...
// DELETE /integrations/supabase/:userId/disconnect — Remove Supabase connection
router.delete('/integrations/supabase/:userId/disconnect', async (req, res) => {
  try {
    const userId = req.params.userId;
    const filePath = path.join(DATA_DIR, `integration-supabase-${userId}.json`);
    await saveJSON(filePath, { status: 'disconnected', disconnectedAt: new Date().toISOString() });
    res.json({ success: true });
  } catch (e) {
...
router.get('/version', (req, res) => {
  res.json({ version: '2.7.0-caesar-instances', timestamp: '2026-02-12T14:30:00Z', features: ['shopify', 'meta', 'meta-oauth', 'google-ads', 'google-ads-oauth', 'whatsapp', 'google-oauth-email', 'postgresql', 'jwt-all-routes', 'ownership-enforcement', 'supabase-integration', 'gemini-imagen', 'caesar-instances'], storageMode: storage.usePg() ? 'postgresql' : 'json' });
});

// ═══════════════════════════════════════════════════════════════════
// CAESAR INSTANCE PROVISIONING
// Dedicated AI COO instance per customer (Phase 3)
// ═══════════════════════════════════════════════════════════════════

// POST /caesar/provision — Provision a new Caesar instance for a customer
router.post('/caesar/provision', async (req, res) => {
  try {
    const { userId, customerName, businessName, plan, anthropicKey, openaiKey, telegramBotToken } = req.body;
    if (!userId) return res.status(400).json({ error: 'userId required' });

    // Store Caesar instance configuration
    const instanceConfig = {
      userId,
      customerName: customerName || 'Customer',
      businessName: businessName || 'Business',
      plan: plan || 'pro', // starter, pro, enterprise
      status: 'provisioning',
      credentials: {
        // Customer's own subscription keys (encrypted in production)
        anthropicKey: anthropicKey || null,
        openaiKey: openaiKey || null,
        telegramBotToken: telegramBotToken || null,
        usePooledKeys: !anthropicKey && !openaiKey // Use Mubyn's keys if customer doesn't provide their own
      },
      config: {
        heartbeatEnabled: true,
        heartbeatIntervalMinutes: 30,
        memoryPersistence: true,
        autonomyLevel: plan === 'enterprise' ? 'high' : plan === 'pro' ? 'medium' : 'low'
      },
      provisionedAt: new Date().toISOString(),
      lastHealthCheck: null,
      containerInfo: null // Will be populated when container is deployed
    };

    await saveJSON(path.join(DATA_DIR, `caesar-instance-${userId}.json`), instanceConfig);

    // In production, this would trigger container deployment via Railway/Fly.io API
    // For now, we store the config and return provisioning status
    res.json({
      success: true,
      status: 'provisioned',
      instanceId: `caesar-${userId}`,
      message: `Caesar instance provisioned for ${businessName}. Container deployment pending.`,
      config: {
        plan: instanceConfig.plan,
        heartbeatEnabled: instanceConfig.config.heartbeatEnabled,
        usePooledKeys: instanceConfig.credentials.usePooledKeys,
        autonomyLevel: instanceConfig.config.autonomyLevel
      }
    });
  } catch (e) {
    console.error('Caesar provision error:', e);
    res.status(500).json({ error: 'Failed to provision Caesar instance', details: e.message });
  }
});

// GET /caesar/status/:userId — Check status of a customer's Caesar instance
router.get('/caesar/status/:userId', async (req, res) => {
  try {
    const userId = req.params.userId;
    const instanceFile = path.join(DATA_DIR, `caesar-instance-${userId}.json`);
    const instance = await loadJSON(instanceFile, null);

    if (!instance) {
      return res.json({ exists: false, status: 'not_provisioned' });
    }
    ...
    res.json({
      ...
      usePooledKeys: instance.credentials?.usePooledKeys
    });
  } catch (e) {
    console.error('Caesar status error:', e);
    res.status(500).json({ error: 'Failed to get Caesar status' });
  }
});

// PUT /caesar/config/:userId — Update Caesar instance configuration
router.put('/caesar/config/:userId', async (req, res) => {
  try {
    const userId = req.params.userId;
    const updates = req.body;
    const instanceFile = path.join(DATA_DIR, `caesar-instance-${userId}.json`);
    ...
    if (!instance) {
      return res.status(404).json({ error: 'Caesar instance not found' });
    }

    // Update allowed fields
    if (updates.anthropicKey !== undefined) instance.credentials.anthropicKey = updates.anthropicKey;
    if (updates.openaiKey !== undefined) instance.credentials.openaiKey = updates.openaiKey;
    if (updates.telegramBotToken !== undefined) instance.credentials.telegramBotToken = updates.telegramBotToken;
    if (updates.heartbeatEnabled !== undefined) instance.config.heartbeatEnabled = updates.heartbeatEnabled;
    if (updates.heartbeatIntervalMinutes !== undefined) instance.config.heartbeatIntervalMinutes = updates.heartbeatIntervalMinutes;
    if (updates.autonomyLevel !== undefined) instance.config.autonomyLevel = updates.autonomyLevel;
    
    // Recalculate usePooledKeys
    instance.credentials.usePooledKeys = !instance.credentials.anthropicKey && !instance.credentials.openaiKey;
    instance.updatedAt = new Date().toISOString();

    await saveJSON(instanceFile, instance);

    res.json({
      success: true,
      message: 'Caesar configuration updated',
      config: instance.config,
      usePooledKeys: instance.credentials.usePooledKeys
    });
  } catch (e) {
    console.error('Caesar config update error:', e);
    res.status(500).json({ error: 'Failed to update Caesar config' });
  }
});

// POST /caesar/health/:userId — Record health check from Caesar instance
router.post('/caesar/health/:userId', async (req, res) => {
  try {
    const userId = req.params.userId;
    ...
    res.json({ success: true, recorded: instance.lastHealthCheck });
  } catch (e) {
    console.error('Caesar health error:', e);
