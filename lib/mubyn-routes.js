...
router.post('/integrations/supabase/:userId/sync', async (req, res) => {
  try {
    ...
    async function fetchTable(table) {
      const r = await fetch(`${data.supabaseUrl}/rest/v1/${encodeURIComponent(table)}?limit=500`, {
    ...
    // 5. byyara_orders → CFO Revenue + Transactions
    if (tables.includes('byyara_orders')) {
      try {
        const orders = await fetchTable('byyara_orders');
        const cfoFile = path.join(DATA_DIR, `cfo-${userId}.json`);
        const cfo = await loadJSON(cfoFile, {});

        // Calculate total revenue from orders
        const totalRevenue = orders.reduce((sum, o) => sum + (parseFloat(o.total) || 0), 0);
        cfo.monthlyRevenue = totalRevenue;
        cfo.netProfit = totalRevenue - (cfo.monthlyExpenses || 0);
        cfo.profitMargin = totalRevenue ? `${Math.round(((totalRevenue - (cfo.monthlyExpenses || 0)) / totalRevenue) * 100)}%` : '0%';

        // Add each order as a CFO transaction
        cfo.transactions = cfo.transactions || [];
        const orderTxns = orders.filter(o => parseFloat(o.total) > 0).map(o => ({
          id: `order-${o.id}`,
          date: o.created_at || new Date().toISOString().slice(0, 10),
          type: 'income',
          category: 'E-commerce Order',
          description: `Order #${o.id}`,
          amount: parseFloat(o.total)
        }));
        // Replace old order txns, keep others
        cfo.transactions = [...cfo.transactions.filter(t => !t.id?.startsWith('order-')), ...orderTxns];
        cfo.dataSource = 'supabase-sync';
        cfo.lastSyncAt = new Date().toISOString();
        await saveJSON(cfoFile, cfo);
        results.synced.push({ table: 'byyara_orders', type: 'cfo-revenue', count: orders.length, totalRevenue });
      } catch (e) {
        results.errors.push({ table: 'byyara_orders', error: e.message });
      }
    }

    // 6. clients_rodmaps → Integration data
    if (tables.includes('clients_rodmaps')) {
      try {
        const roadmaps = await fetchTable('clients_rodmaps');
        const rmFile = path.join(DATA_DIR, `integration-supabase-roadmaps-${userId}.json`);
        await saveJSON(rmFile, { roadmaps, syncedAt: new Date().toISOString() });
        results.synced.push({ table: 'clients_rodmaps', type: 'roadmaps-data', count: roadmaps.length });
      } catch (e) {
        results.errors.push({ table: 'clients_rodmaps', error: e.message });
      }
    }

    // 7. chatbot_conversations → CS context
    if (tables.includes('chatbot_conversations')) {
      try {
        const conversations = await fetchTable('chatbot_conversations');
        const csFile = path.join(DATA_DIR, `integration-supabase-cs-${userId}.json`);
        await saveJSON(csFile, { conversationCount: conversations.length, conversations, syncedAt: new Date().toISOString() });
        results.synced.push({ table: 'chatbot_conversations', type: 'cs-context', count: conversations.length });
      } catch (e) {
        results.errors.push({ table: 'chatbot_conversations', error: e.message });
      }
    }

    // 8. pending_invoice_jobs → CFO Transactions
