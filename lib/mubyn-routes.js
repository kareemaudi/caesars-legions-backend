  ...
  try {
    ...
    if (userId) {
      try {
        ...
        // ── Inject REAL financial data when user asks about money/financials ──
        const msgLower = message.toLowerCase();
        const financialKeywords = ['revenue', 'financial', 'financials', 'profit', 'expenses', 'money', 'income', 'transactions', 'payroll', 'salary', 'budget', 'cash', 'burn', 'p&l', 'pnl', 'expense', 'cost', 'costs', 'earnings', 'margin', 'forecast', 'projection', 'projections'];
        const teamKeywords = ['team', 'employees', 'staff', 'who works', 'members', 'payroll', 'salary', 'salaries', 'headcount', 'hire', 'hiring'];
        const isFinancialQuery = financialKeywords.some(kw => msgLower.includes(kw));
        const isTeamQuery = teamKeywords.some(kw => msgLower.includes(kw));

        if (isFinancialQuery && cfo) {
          const catBreakdown = cfo.categoryBreakdown ? Object.entries(cfo.categoryBreakdown)
            .map(([cat, data]) => `${cat}: income $${(data.income||0).toLocaleString()}, expenses $${Math.abs(data.expenses||0).toLocaleString()} (${data.count} txns)`)
            .join('; ') : 'N/A';
          userContext += `\n\n[FINANCIAL CONTEXT FROM SUPABASE - REAL DATA]
Total Revenue: $${(cfo.totalRevenue||cfo.monthlyRevenue||0).toLocaleString()} | Total Expenses: $${(cfo.totalExpenses||cfo.monthlyExpenses||0).toLocaleString()} | Net Profit: $${(cfo.netProfit||0).toLocaleString()} (${cfo.profitMargin || 'N/A'} margin)
Monthly Avg Revenue: $${(cfo.monthlyRevenue||0).toLocaleString()} | Monthly Avg Expenses: $${(cfo.monthlyExpenses||0).toLocaleString()}
${cfo.transactionCount ? `Transaction count: ${cfo.transactionCount}` : ''}
${catBreakdown !== 'N/A' ? `Category breakdown: ${catBreakdown}` : ''}
${cfo.dataSource ? `Data source: ${cfo.dataSource}` : ''}
${cfo.lastSyncAt ? `Last synced: ${cfo.lastSyncAt}` : ''}
IMPORTANT: Use these REAL numbers when answering. Do NOT make up financial figures.`;
        }

        if (isTeamQuery) {
          try {
            const teamData = await loadJSON(path.join(DATA_DIR, `integration-supabase-team-${userId}.json`), null);
            if (teamData && teamData.team_members && teamData.team_members.length > 0) {
              ...
              const memberList = members.map(m => `- ${m.name || 'Unknown'}: ${m.position || m.role || 'Team'} ($${(parseFloat(m.amount)||0).toLocaleString()}/mo)`).join('\n');
              userContext += `\n\n[TEAM DATA FROM SUPABASE - REAL DATA]
Team Size: ${members.length} members | Monthly Payroll: $${totalPayroll.toLocaleString()}
${memberList}
Source: ${teamData.source_table || 'supabase'} | Synced: ${teamData.syncedAt || 'N/A'}
IMPORTANT: Use these REAL team details when answering. Do NOT make up employee information.`;
            }
          } catch (_teamErr) { /* team data not available, that's ok */ }
        }
...
router.get('/version', (req, res) => {
  res.json({ version: '2.6.0-supabase', timestamp: '2026-02-11T23:00:00Z', features: ['shopify', 'meta', 'meta-oauth', 'google-ads', 'google-ads-oauth', 'whatsapp', 'google-oauth-email', 'postgresql', 'jwt-all-routes', 'ownership-enforcement', 'supabase-integration'], storageMode: storage.usePg() ? 'postgresql' : 'json' });
});
