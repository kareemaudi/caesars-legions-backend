...
router.post('/integrations/supabase/:userId/sync', async (req, res) => {
  try {
    ...
    // 4. team_members → stored for context
    if (tables.includes('team_members')) {
      try {
        const members = await fetchTable('team_members');
        const teamFile = path.join(DATA_DIR, `integration-supabase-team-${userId}.json`);
        await saveJSON(teamFile, { team_members: members, syncedAt: new Date().toISOString() });
        results.synced.push({ table: 'team_members', type: 'team-context', count: members.length });
      } catch (e) {
