...
// Metrics: Integrated with MetricsTracker for system-wide observability
// ============================================================================
...
class ABTestManager {
  ...
  async createTest(testConfig) {
    ...
    // Track test creation in metrics
    if (this.trackMetrics) {
      this.metrics.trackEvent('ab_test', 'test_created', {
        testId: test.id,
        testType: test.type,
  ...
  async recordEvent(testId, recipientId, eventType, metadata = {}) {
    ...
    // Track event in central metrics
    if (this.trackMetrics) {
      this.metrics.trackEvent('ab_test', eventType, {
        testId,
        variantId: assignment.variantId,
        recipientId
      });
  ...
  async endTest(testId, reason, winnerId = null) {
    ...
    // Track test completion in metrics
    if (this.trackMetrics) {
      const winningVariant = test.variants.find(v => v.id === winnerId);
      this.metrics.trackEvent('ab_test', 'test_completed', {
        testId,
        winnerId,
        winnerName: winningVariant?.name,
        endReason: reason,
        testType: test.type,
        totalSent: test.variants.reduce((sum, v) => sum + v.stats.sent, 0),
        durationDays: ((Date.now() - new Date(test.createdAt)) / (1000 * 60 * 60 * 24)).toFixed(1)
      });
    }
