...
const query = {
  ...
  getClientByEmail: (email) => {
    return db.clients.find(c => c.email.toLowerCase() === email.toLowerCase());
  },
  ...
  updateClient: (id, updates) => {
    const client = db.clients.find(c => c.id === parseInt(id));
    if (client) {
  ...
  insertEmailSent: (data) => {
    ...
    const defaultSentAt = Math.floor(Date.now() / 1000);
    ...
    const email = {
      ...
      body: data.body,
      sent_at: data.sent_at || defaultSentAt, // Use provided or default
