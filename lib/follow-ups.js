...
const DEFAULT_CONFIG = {
  ...
  rateLimitMs: 1000, // Delay between sends (1000ms = 1 second)
...
async function processFollowUps(options = {}) {
  ...
  for (const client of clients) {
    ...
    for (const email of recentEmails) {
      ...
      // Double-check reply status (safety check)
      if (hasLeadReplied(email.lead_id, client.id)) {
        if (config.verbose) {
          console.log(`   ⏭️ Skipping ${email.lead_email} - has replied`);
        }
        results.skipped++;
        continue;
      }
      ...
      const followUpsSent = countFollowUpsSent(email.lead_id, client.id);
...
async function sendFollowUpEmail(client, originalEmail, daysSince, dryRun = true, config = DEFAULT_CONFIG) {
  ...
  await new Promise(resolve => setTimeout(resolve, config.rateLimitMs || 1000));
}

/**
 * Test if a lead has replied (safety check before sending follow-up)
 * Checks both replied flag and replies table
 */
function hasLeadReplied(leadId, clientId) {
  const rawDb = require('fs').existsSync(require('path').join(__dirname, '..', 'data', 'legions.json'))
    ? JSON.parse(require('fs').readFileSync(require('path').join(__dirname, '..', 'data', 'legions.json'), 'utf8'))
    : { emails_sent: [], replies: [] };
  
  // Check if any email to this lead is marked as replied
  const leadEmails = rawDb.emails_sent.filter(e => 
    e.lead_id === leadId && e.client_id === clientId
  );
  
  if (leadEmails.some(e => e.replied)) {
    return true;
  }
  
  // Also check replies table
  if (rawDb.replies && rawDb.replies.some(r => r.lead_id === leadId)) {
    return true;
  }
 ...
 * Optimized to avoid loading all emails from database
 */
function countFollowUpsSent(leadId, clientId) {
  // Access raw database to avoid expensive getRecentEmails call
  const dbModule = require('./db');
  const rawDb = require('fs').existsSync(require('path').join(__dirname, '..', 'data', 'legions.json'))
    ? JSON.parse(require('fs').readFileSync(require('path').join(__dirname, '..', 'data', 'legions.json'), 'utf8'))
    : { emails_sent: [] };
  
  // Filter by lead_id directly from raw data
  const leadEmails = rawDb.emails_sent.filter(e => 
    e.lead_id === leadId && e.client_id === clientId
  );
