...
async function handleEmailReplied(event) {
  ...
  // Create reply record
  const sentiment = await analyzeSentiment(reply_text);
  ...
  console.log(`✓ Reply received: ${email} (${sentiment})`);
...
async function handleUnsubscribed(event) {
  ...
  // TODO: Add to global unsubscribe list (never email again)
}

/**
 * Analyze sentiment of reply text (using GPT-4)
 */
async function analyzeSentiment(text) {
  // Placeholder - would use OpenAI
  const lowerText = text.toLowerCase();
  
  if (lowerText.includes('interested') || lowerText.includes('yes') || lowerText.includes('tell me more')) {
    return 'positive';
  } else if (lowerText.includes('not interested') || lowerText.includes('no') || lowerText.includes('remove')) {
    return 'negative';
  }
