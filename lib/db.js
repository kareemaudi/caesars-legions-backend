// Database Layer for Caesar's Legions
// Tries SQLite first, falls back to JSON file storage if better-sqlite3 unavailable
...
try {
  ...
  console.log('⚠️ better-sqlite3 not available, falling back to JSON storage');
}

if (!useSQLite) {
  module.exports = require('./db-json-backup');
} else {
// ---- SQLite mode ----
...
const statements = {
  insertClient: db.prepare(`
    INSERT INTO clients (email, name, company, target_audience, value_prop, website, status, monthly_quota, plan, price, onboarding_data)
    VALUES (@email, @name, @company, @target_audience, @value_prop, @website, @status, @monthly_quota, @plan, @price, @onboarding_data)
  `),
