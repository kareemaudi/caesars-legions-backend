 ...
 */
async function createStripeCheckout({ clientId, email, name, company, price }) {
  const STRIPE_SECRET_KEY = process.env.STRIPE_SECRET_KEY;
  
  if (!STRIPE_SECRET_KEY) {
    // Development mode: Return mock URL
    console.warn('⚠️ STRIPE_SECRET_KEY not configured. Using development mode.');
    const baseUrl = process.env.BASE_URL || 'http://localhost:3000';
    return `${baseUrl}/signup.html?success=true&session_id=dev_${Date.now()}&client_id=${clientId}`;
  }
  ...
  try {
      ...
      line_items: [
        {
          price_data: {
            currency: 'usd',
            product_data: {
              name: "Caesar's Legions - Early Bird",
              description: '100 personalized cold emails/week + automated follow-ups',
              images: ['https://promptabusiness.com/caesar-logo.png'] // Optional: Add logo
            },
            recurring: {
              interval: 'month'
            },
            unit_amount: price * 100 // $250 = 25000 cents
          },
          quantity: 1
        }
      ],
      success_url: `${baseUrl}/signup.html?success=true&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${baseUrl}/signup.html?canceled=true`,
      metadata: {
        client_id: clientId,
        company: company,
        plan: 'early_bird'
    ...
    console.log(`✅ Stripe checkout created for ${email}: ${session.url}`);

    return session.url;
  } catch (error) {
    console.error('❌ Stripe error:', error.message);
    throw new Error('Failed to create payment session. Please contact support.');
  }
}
