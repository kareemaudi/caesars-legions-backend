...
// === LEAD FINDER (PRIMARY — WE find leads for the client) ===
app.post('/api/leads/find', authMiddleware, async (req, res) => {
  ...
  try {
    // Use Brave Search API to find businesses (reliable, no API key restrictions)
    const BRAVE_KEY = process.env.BRAVE_API_KEY || '';
    const query = `${industry} ${title || 'businesses'} in ${location} contact email phone`;
    const braveUrl = `https://api.search.brave.com/res/v1/web/search?q=${encodeURIComponent(query)}&count=20`;
    ...
    if (BRAVE_KEY) {
      try {
        const braveRes = await fetch(braveUrl, { headers: { 'X-Subscription-Token': BRAVE_KEY, 'Accept': 'application/json' } });
        const braveData = await braveRes.json();
        if (braveData.web && braveData.web.results) {
          searchResults = braveData.web.results;
        }
      } catch (e) { console.log('Brave search failed:', e.message); }
    }

    // Also try a Google Maps embed search (no API key needed for basic search)
    const mapsQuery = `${industry} in ${location}`;
    const googleUrl = `https://www.google.com/search?q=${encodeURIComponent(mapsQuery + ' site:google.com/maps')}&num=20`;

    // Parse search results into leads
    const leads = {};
    
    for (const result of searchResults) {
      const leadId = crypto.randomBytes(4).toString('hex');
      // Extract business name from title (before ' - ' or ' | ')
      const name = (result.title || '').split(/\s[-|]\s/)[0].trim();
      // Try to find email in description
      const emailMatch = (result.description || '').match(/[\w.-]+@[\w.-]+\.\w+/);
      // Try to find phone
      const phoneMatch = (result.description || '').match(/[\+]?[(]?\d{1,4}[)]?[-\s.]?\d{1,4}[-\s.]?\d{1,9}/);
    ...
    // If we didn't get enough from Brave, add placeholder leads from the search request
    // This signals to the system that we need manual enrichment
    if (Object.keys(leads).length < 5) {
      // Save the search request for manual fulfillment
      const requestFile = path.join(DATA_DIR, `lead_requests_${req.clientId}.json`);
      const requests = loadJSON(requestFile);
      requests[Date.now()] = { industry, location, title, count: requestedCount, pitch, status: 'pending', requestedAt: new Date().toISOString() };
      saveJSON(requestFile, requests);
      ...
      if (Object.keys(leads).length === 0) {
        return res.json({ 
          ...
          leadsFound: 0, 
          total: 0,
          message: 'Your lead request has been queued. Our AI is finding and verifying leads for you — check back in 1-2 hours.'
        });
      }
    }
    ...
    const existing = loadJSON(leadsFile);
    const merged = { ...existing, ...leads };
    saveJSON(leadsFile, merged);

    // Step 6: AUTO-CREATE CAMPAIGN for these leads
    const campaigns = loadJSON(CAMPAIGNS_FILE);
    const campaignId = 'campaign_' + crypto.randomBytes(8).toString('hex');
    const pitchText = pitch || `We help ${industry} businesses grow with AI-powered solutions`;
    
    campaigns[campaignId] = {
      campaignId,
      clientId: req.clientId,
      name: `${industry} in ${location} — Outreach`,
      subject: `Quick question about {{company}}`,
      body: `<p>Hi{{firstName ? ' ' + firstName : ''}},</p>
<p>I came across <strong>{{company}}</strong> and was impressed by what you've built.</p>
<p>${pitchText}</p>
<p>Would you be open to a quick 10-minute call this week to see if we can help?</p>
<p>Best regards</p>`,
      leadEmails: Object.values(leads).filter(l => l.email).map(l => l.email),
      schedule: 'immediate',
      status: 'ready',
      createdAt: new Date().toISOString(),
      autoGenerated: true,
      searchCriteria: { industry, location, title, count: requestedCount },
      stats: { sent: 0, opened: 0, replied: 0, bounced: 0 }
    };
    saveJSON(CAMPAIGNS_FILE, campaigns);

    console.log(`🎯 Found ${Object.keys(leads).length} leads + auto-campaign ${campaignId} for client ${req.clientId}: ${industry} in ${location}`);
    res.json({ success: true, leadsFound: Object.keys(leads).length, total: Object.keys(merged).length, campaignId });
  } catch (err) {
    console.error('Lead find error:', err);
    res.json({ success: false, error: 'Search queued. Our team will find leads for you within a few hours.' });
  }
});

// Upload leads (CSV or JSON array) — secondary option
